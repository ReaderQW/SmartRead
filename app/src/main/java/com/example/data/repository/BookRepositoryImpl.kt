...
class BookRepositoryImpl(
    ...
    override val allBooks: Flow<List<Book>> = bookDao.observeBooks().map { entities ->
    ...
    override suspend fun addBook(book: Book): Int = withContext(Dispatchers.IO) {
        val id = bookDao.insertBook(book.toEntity()).toInt()
        knowledgeDao.insertNode(KnowledgeNode("book_$id", id, book.title, "Book", 1.8f).toEntity())
        id
    }
